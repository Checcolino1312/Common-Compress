...
public final class JarTestCase extends AbstractTestCase {
    ...
    public void testJarUnarchive() throws Exception {
        ...
        try (InputStream is = Files.newInputStream(input.toPath());
            ...
            Files.copy(in, o.toPath());
            // Asserzione
            assertTrue(o.exists()); // Verifica che il file sia stato copiato correttamente
    ...
    public void testJarUnarchiveAll() throws Exception {
        final File input = getFile("bla.jar");
        boolean fileUnarchived = false; // Flag per tenere traccia dell'unarchiviazione di almeno un file
        ...
        try (InputStream is = Files.newInputStream(input.toPath());
            ...
            while (entry != null) {
                ...
                // Imposta il flag a true se almeno un file è stato unarchiviato correttamente
                fileUnarchived = true;
            }
        }

        // Verifica che almeno un file sia stato unarchiviato correttamente
        Assert.assertTrue(fileUnarchived);
    }
