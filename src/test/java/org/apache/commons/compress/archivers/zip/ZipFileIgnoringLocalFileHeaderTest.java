...
public class ZipFileIgnoringLocalFileHeaderTest {
    ...
    public void testZipUnarchive() throws Exception {
        boolean fileExtracted = false; // Indicatore per verificare se almeno un file è stato estratto
        ...
        try (final ZipFile zf = openZipWithoutLFH("bla.zip")) {
            for (final Enumeration<ZipArchiveEntry> e = zf.getEntries(); e.hasMoreElements();) {
                ...
                fileExtracted = true; // Imposta l'indicatore su true se almeno un file è stato estratto
            }
        }

        // Verifica se almeno un file è stato estratto correttamente
        assertTrue(fileExtracted);
    }
