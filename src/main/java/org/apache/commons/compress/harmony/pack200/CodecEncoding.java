...
public class CodecEncoding {
    ...
    public static Codec getCodec(final int value, final InputStream in, final Codec defaultCodec)
        ...
        if (canonicalCodec.length != 116) {
            throw new AssertionError("Canonical encodings have been incorrectly modified");
        }
        if (value < 0) {
        ...
        if (value == 116) {
            return decodeBHSDCodec(in);
        }
        if (value >= 117 && value <= 140) { // Run codec
            return decodeRunCodec(value, in, defaultCodec);
        }
        if ((value < 141) || (value > 188)) {
            throw new Pack200Exception("Invalid codec encoding byte (" + value + ") found");
        }
        return decodePopulationCodec(value, in, defaultCodec);
    }

    private static BHSDCodec decodeBHSDCodec(final InputStream in) throws IOException{
        int code = in.read();
        if (code == -1) {
    ...
    private static RunCodec decodeRunCodec(final int value, final InputStream in, final Codec defaultCodec)
            throws IOException, Pack200Exception {
    ...
    private static PopulationCodec decodePopulationCodec(final int value, final InputStream in, final Codec defaultCodec)
            throws IOException, Pack200Exception {
    ...
    public static int[] getSpecifier(final Codec codec, final Codec defaultForBand) {
        ...
        if (canonicalCodecsToSpecifiers == null) {
            initializeCanonicalCodecsToSpecifiersMap();
        }
        ...
        if (codec instanceof BHSDCodec) {
            return getBHSDSpecifier((BHSDCodec) codec);
        }

        if (codec instanceof RunCodec) {
            return getRunCodecSpecifier((RunCodec) codec, defaultForBand);
        }
    ...
    private static int[] getBHSDSpecifier(final BHSDCodec bhsdCodec) {
    ...
    private static int[] getRunCodecSpecifier(final RunCodec runCodec, final Codec defaultForBand) {
        ...
        System.arraycopy(aSpecifier, 0, specifier, index, aSpecifier.length);
        index += aSpecifier.length;
        System.arraycopy(bSpecifier, 0, specifier, index, bSpecifier.length);
        return specifier;
    }

    private static int[] getPopulationCodecSpecifier(final PopulationCodec populationCodec, final Codec defaultForBand) {
        ...
        if (favoured != null && (tokenCodec == Codec.BYTE1 || (tokenCodec instanceof BHSDCodec && ((BHSDCodec) tokenCodec).getS() == 0))) {
            if (tokenCodec == Codec.BYTE1) {
                ...
                final int index = Arrays.binarySearch(possibleLValues, l);
                if (index != -1) {
                    tDefL = index + 1;
                }
            }
        ...
        specifier[0] = first;
        int index = 1;
        System.arraycopy(favouredSpecifier, 0, specifier, index, favouredSpecifier.length);
        index += favouredSpecifier.length;
        System.arraycopy(tokenSpecifier, 0, specifier, index, tokenSpecifier.length);
        index += tokenSpecifier.length;
        System.arraycopy(unfavouredSpecifier, 0, specifier, index, unfavouredSpecifier.length);
        return specifier;
    }
