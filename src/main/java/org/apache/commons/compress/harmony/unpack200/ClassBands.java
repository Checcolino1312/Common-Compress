...
public class ClassBands extends BandSet {
    ...
    private int countCalls(int[][] codeAttrIndexes, AttributeLayoutMap attrMap) {
        int callCount = 0;
    ...
    private void processAttributes(AttributeLayoutMap attrMap, long[] codeFlags, int limit, AttributeLayout[] otherLayouts, int[] counts) {
        for (int i = 0; i < limit; i++) {
    ...
    private void parseCodeAttrBands(final InputStream in, final int codeFlagsCount)
        ...
        int callCount = countCalls(codeAttrIndexes, attrMap);
        ...
        final int[] counts = new int[limit + 1];
        final List<Attribute>[] otherAttributes = new List[limit + 1];
        processAttributes(attrMap, codeFlags, limit, otherLayouts, counts);
        for (int i = 0; i < counts.length; i++) {
            if (counts[i] <= 0)
                continue;
