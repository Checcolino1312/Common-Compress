...
public class CpBands extends BandSet {
    ...
    public <T extends CPConstant<T>> T getConstant(final Object value) {
        return objectsToCPConstant.computeIfAbsent(value, this::createConstant);
    }

    private CPConstant<?> createConstant(final Object value) {
        if (value instanceof Integer) {
            CPInt constant = new CPInt(((Integer) value).intValue());
            cp_Int.add(constant);
            return constant;
        } else if (value instanceof Long) {
            CPLong constant = new CPLong(((Long) value).longValue());
            cp_Long.add(constant);
            return constant;
        } else if (value instanceof Float) {
            CPFloat constant = new CPFloat(((Float) value).floatValue());
            cp_Float.add(constant);
            return constant;
        } else if (value instanceof Double) {
            CPDouble constant = new CPDouble(((Double) value).doubleValue());
            cp_Double.add(constant);
            return constant;
        } else if (value instanceof String) {
            CPString constant = new CPString(getCPUtf8((String) value));
    ...
    public CPClass getCPClass(String className) {
        ...
        return stringsToCpClass.computeIfAbsent(className, this::createCPClass);
    }

    private CPClass createCPClass(String className) {
        CPUTF8 cpUtf8 = getCPUtf8(className);
        CPClass cpClass = new CPClass(cpUtf8);
    ...
    public CPMethodOrField getCPField(final CPClass cpClass, final String name, final String desc) {
        ...
        return stringsToCpField.computeIfAbsent(key, k -> {
            CPNameAndType nAndT = getCPNameAndType(name, desc);
            CPMethodOrField cpField = new CPMethodOrField(cpClass, nAndT);
            cp_Field.add(cpField);
            return cpField;
        });
    }
    ...
    public CPMethodOrField getCPIMethod(final CPClass cpClass, final String name, final String desc) {
        final String key = cpClass.toString() + ":" + name + ":" + desc;
        return stringsToCpIMethod.computeIfAbsent(key, k -> {
            CPNameAndType nAndT = getCPNameAndType(name, desc);
            CPMethodOrField cpIMethod = new CPMethodOrField(cpClass, nAndT);
            cp_Imethod.add(cpIMethod);
            return cpIMethod;
        });
    }
    ...
    public CPMethodOrField getCPMethod(final CPClass cpClass, final String name, final String desc) {
        ...
        return stringsToCpMethod.computeIfAbsent(key, k -> {
            CPNameAndType nAndT = getCPNameAndType(name, desc);
            CPMethodOrField cpMethod = new CPMethodOrField(cpClass, nAndT);
            cp_Method.add(cpMethod);
            return cpMethod;
        });
    }
    ...
    public CPNameAndType getCPNameAndType(final String name, final String signature) {
        final String key = name + ":" + signature;
        return stringsToCpNameAndType.computeIfAbsent(key, k -> {
            CPNameAndType nameAndType = new CPNameAndType(getCPUtf8(name), getCPSignature(signature));
    ...
    public CPSignature getCPSignature(final String signature) {
        ...
        return stringsToCpSignature.computeIfAbsent(signature, s -> {
            ...
            CPSignature cpSignature = new CPSignature(signature, signatureUTF8, cpClasses);
            cp_Signature.add(cpSignature);
            return cpSignature;
        });
    }
    ...
    public CPUTF8 getCPUtf8(final String utf8) {
        ...
        return stringsToCpUtf8.computeIfAbsent(utf8, u -> {
    ...
    private static final String PREFIX = "Writing ";

    private static final String LOG_PREFIX = "Wrote ";

    private void writeCpClass(final OutputStream out) throws IOException, Pack200Exception {
        PackingUtils.log(PREFIX + cp_Class.size() + " Class entries...");
        ...
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from cpClass[" + cpClass.length + "]");
    }

    private void writeCpDescr(final OutputStream out) throws IOException, Pack200Exception {
        PackingUtils.log( PREFIX + cp_Descr.size() + " Descriptor entries...");
        ...
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from cp_Descr_Name[" + cpDescrName.length + "]");

        encodedBand = encodeBandInt("cp_Descr_Type", cpDescrType, Codec.UDELTA5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from cp_Descr_Type[" + cpDescrType.length + "]");
    }

    private void writeCpDouble(final OutputStream out) throws IOException, Pack200Exception {
        PackingUtils.log( PREFIX + cp_Double.size() + " Double entries...");
        ...
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from cp_Double_hi[" + highBits.length + "]");

        encodedBand = encodeBandInt("cp_Double_lo", loBits, Codec.DELTA5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from cp_Double_lo[" + loBits.length + "]");
    }

    private void writeCpFloat(final OutputStream out) throws IOException, Pack200Exception {
        PackingUtils.log( PREFIX + cp_Float.size() + " Float entries...");
        ...
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from cp_Float[" + cpFloat.length + "]");
    }

    private void writeCpInt(final OutputStream out) throws IOException, Pack200Exception {
        PackingUtils.log( PREFIX + cp_Int.size() + " Integer entries...");
        ...
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from cp_Int[" + cpInt.length + "]");
    }

    private void writeCpLong(final OutputStream out) throws IOException, Pack200Exception {
        PackingUtils.log( PREFIX + cp_Long.size() + " Long entries...");
        ...
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from cp_Long_hi[" + highBits.length + "]");

        encodedBand = encodeBandInt("cp_Long_lo", loBits, Codec.DELTA5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from cp_Long_lo[" + loBits.length + "]");
    }

    private void writeCpMethodOrField(final Set<CPMethodOrField> cp, final OutputStream out, final String name)
        throws IOException, Pack200Exception {
        PackingUtils.log( PREFIX + cp.size() + " Method and Field entries...");
        ...
        out.write(encodedBand);
        PackingUtils.log(
                LOG_PREFIX + encodedBand.length + " bytes from " + name + "_class[" + cp_methodOrField_class.length + "]");

        encodedBand = encodeBandInt(name + "_desc", cp_methodOrField_desc, Codec.UDELTA5);
        out.write(encodedBand);
        PackingUtils
            .log(LOG_PREFIX + encodedBand.length + " bytes from " + name + "_desc[" + cp_methodOrField_desc.length + "]");
    }

    private void writeCpSignature(final OutputStream out) throws IOException, Pack200Exception {
        PackingUtils.log( PREFIX + cp_Signature.size() + " Signature entries...");
        ...
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from cpSignatureForm[" + cpSignatureForm.length + "]");

        encodedBand = encodeBandInt("cpSignatureClasses", cpSignatureClasses, Codec.UDELTA5);
        out.write(encodedBand);
        PackingUtils
            .log(LOG_PREFIX + encodedBand.length + " bytes from cpSignatureClasses[" + cpSignatureClasses.length + "]");
    }

    private void writeCpString(final OutputStream out) throws IOException, Pack200Exception {
        PackingUtils.log( PREFIX + cp_String.size() + " String entries...");
        ...
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from cpString[" + cpString.length + "]");
    }

    private void writeCpUtf8(final OutputStream out) throws IOException, Pack200Exception {
        PackingUtils.log( PREFIX + cp_Utf8.size() + " UTF8 entries...");
        ...
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from cpUtf8Prefix[" + cpUtf8Prefix.length + "]");

        encodedBand = encodeBandInt("cpUtf8Suffix", cpUtf8Suffix, Codec.UNSIGNED5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from cpUtf8Suffix[" + cpUtf8Suffix.length + "]");

        encodedBand = encodeBandInt("cpUtf8Chars", cpUtf8Chars, Codec.CHAR3);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from cpUtf8Chars[" + cpUtf8Chars.length + "]");

        encodedBand = encodeBandInt("cpUtf8BigSuffix", cpUtf8BigSuffix, Codec.DELTA5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from cpUtf8BigSuffix[" + cpUtf8BigSuffix.length + "]");

        for (int i = 0; i < cpUtf8BigChars.length; i++) {
            encodedBand = encodeBandInt("cpUtf8BigChars " + i, cpUtf8BigChars[i], Codec.DELTA5);
            out.write(encodedBand);
            PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from cpUtf8BigChars" + i + "["
