...
public class BcBands extends BandSet {
    ...
    public void pack(final OutputStream out) throws IOException, Pack200Exception {
        ...
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from bcCodes[" + bcCodes.size() + "]");

        encodedBand = encodeBandInt("bcCaseCount", bcCaseCount.toArray(), Codec.UNSIGNED5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from bcCaseCount[" + bcCaseCount.size() + "]");

        encodedBand = encodeBandInt("bcCaseValue", bcCaseValue.toArray(), Codec.DELTA5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from bcCaseValue[" + bcCaseValue.size() + "]");

        encodedBand = encodeBandInt("bcByte", bcByte.toArray(), Codec.BYTE1);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from bcByte[" + bcByte.size() + "]");

        encodedBand = encodeBandInt("bcShort", bcShort.toArray(), Codec.DELTA5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from bcShort[" + bcShort.size() + "]");

        encodedBand = encodeBandInt("bcLocal", bcLocal.toArray(), Codec.UNSIGNED5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from bcLocal[" + bcLocal.size() + "]");

        encodedBand = encodeBandInt("bcLabel", integerListToArray(bcLabel), Codec.BRANCH5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from bcLabel[" + bcLabel.size() + "]");

        encodedBand = encodeBandInt("bcIntref", cpEntryListToArray(bcIntref), Codec.DELTA5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from bcIntref[" + bcIntref.size() + "]");

        encodedBand = encodeBandInt("bcFloatRef", cpEntryListToArray(bcFloatRef), Codec.DELTA5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from bcFloatRef[" + bcFloatRef.size() + "]");

        encodedBand = encodeBandInt("bcLongRef", cpEntryListToArray(bcLongRef), Codec.DELTA5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from bcLongRef[" + bcLongRef.size() + "]");

        encodedBand = encodeBandInt("bcDoubleRef", cpEntryListToArray(bcDoubleRef), Codec.DELTA5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from bcDoubleRef[" + bcDoubleRef.size() + "]");

        encodedBand = encodeBandInt("bcStringRef", cpEntryListToArray(bcStringRef), Codec.DELTA5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from bcStringRef[" + bcStringRef.size() + "]");

        encodedBand = encodeBandInt("bcClassRef", cpEntryOrNullListToArray(bcClassRef), Codec.UNSIGNED5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from bcClassRef[" + bcClassRef.size() + "]");

        encodedBand = encodeBandInt("bcFieldRef", cpEntryListToArray(bcFieldRef), Codec.DELTA5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from bcFieldRef[" + bcFieldRef.size() + "]");

        encodedBand = encodeBandInt("bcMethodRef", cpEntryListToArray(bcMethodRef), Codec.UNSIGNED5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from bcMethodRef[" + bcMethodRef.size() + "]");

        encodedBand = encodeBandInt("bcIMethodRef", cpEntryListToArray(bcIMethodRef), Codec.DELTA5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from bcIMethodRef[" + bcIMethodRef.size() + "]");

        encodedBand = encodeBandInt("bcThisField", integerListToArray(bcThisField), Codec.UNSIGNED5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from bcThisField[" + bcThisField.size() + "]");

        encodedBand = encodeBandInt("bcSuperField", integerListToArray(bcSuperField), Codec.UNSIGNED5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from bcSuperField[" + bcSuperField.size() + "]");

        encodedBand = encodeBandInt("bcThisMethod", integerListToArray(bcThisMethod), Codec.UNSIGNED5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from bcThisMethod[" + bcThisMethod.size() + "]");

        encodedBand = encodeBandInt("bcSuperMethod", integerListToArray(bcSuperMethod), Codec.UNSIGNED5);
        out.write(encodedBand);
        PackingUtils.log(LOG_PREFIX + encodedBand.length + " bytes from bcSuperMethod[" + bcSuperMethod.size() + "]");
...
//            bcSuperField.add(cpField);
    ...
    public void visitMethodInsn(int opcode, final String owner, final String name, final String desc) {
        ...
        switch (opcode) {
            ...
            case 184: // invokestatic
                handleMethodInvocation(opcode, owner, name, desc);
                break;
            case 185: // invokeinterface
                handleInterfaceMethodInvocation(owner, name, desc);
                break;
        }
    ...
    private void handleMethodInvocation(int opcode, String owner, String name, String desc) {
        boolean aload_0 = false;
        ...
        if (owner.equals(currentClass)) {
            handleCurrentClassMethodInvocation(opcode, owner, name, desc);
        } else if (owner.equals(superClass)) {
            handleSuperClassMethodInvocation(opcode, owner, name, desc);
        } else {
            handleOtherClassMethodInvocation(aload_0, opcode, owner, name, desc);
        }
    }

    private void handleCurrentClassMethodInvocation(int opcode, String owner, String name, String desc) {
        opcode += 24;

        if (name.equals(INIT_METHOD_NAME) && opcode == 207) {
            opcode = 230;
    ...
    private void handleSuperClassMethodInvocation(int opcode, String owner, String name, String desc) {
        opcode += 38;

        if (name.equals(INIT_METHOD_NAME) && opcode == 221) {
            opcode = 231;
            bcInitRef.add(cpBands.getCPMethod(owner, name, desc));
        } else {
    ...
    private void handleOtherClassMethodInvocation(boolean aload_0, int opcode, String owner, String name, String desc) {
        if (aload_0) {
            opcode -= 7;
            bcCodes.add(ALOAD_0);
        }

        if (name.equals(INIT_METHOD_NAME) && opcode == 183 && owner.equals(currentNewClass)) {
            opcode = 232;
            bcInitRef.add(cpBands.getCPMethod(owner, name, desc));
        } else {
    ...
    private void handleInterfaceMethodInvocation(String owner, String name, String desc) {
        byteCodeOffset += 2;
