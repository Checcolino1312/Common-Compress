...
public abstract class BandSet {
        ...
        public BandData(final int[] band) {
            ...
            for (int i = 0; i < band.length; i++) {
                ...
                if (i != 0) {
                    ...
                    updateSmallestDelta(delta);
                    updateLargestDelta(delta);
                    updateDeltaIsAscending(delta);
                    updateAverageAbsoluteDelta(delta, band.length);
                    updateSmallDeltaCount(delta);
                } else {
                    initializeDeltaValues(band[0]);
                }
                updateAverageAbsoluteValue(band[i], band.length);
                calculateDistinctValues(band[i]);
            }
        }

        private void updateSmallestDelta(int delta) {
            if (delta < smallestDelta) {
                smallestDelta = delta;
            }
        }

        private void updateLargestDelta(int delta) {
            if (delta > largestDelta) {
                largestDelta = delta;
            }
        }

        private void updateDeltaIsAscending(int delta) {
            if (delta >= 0) {
                deltaIsAscending++;
            }
        }

        private void updateAverageAbsoluteDelta(int delta, int length) {
            averageAbsoluteDelta += (double) Math.abs(delta) / (double) (length - 1);
        }

        private void updateSmallDeltaCount(int delta) {
            if (Math.abs(delta) < 256) {
                smallDeltaCount++;
            }
        }

        private void initializeDeltaValues(int value) {
            smallestDelta = value;
            largestDelta = value;
        }

        private void updateAverageAbsoluteValue(int value, int length) {
            averageAbsoluteValue += (double) Math.abs(value) / (double) length;
        }

        private void calculateDistinctValues(int value) {
            if (effort > 3) {
                if (distinctValues == null) {
                    distinctValues = new HashMap<>();
                }
                final Integer integer = Integer.valueOf(value);
                Integer count = distinctValues.get(integer);
                if (count == null) {
                    count = Integer.valueOf(1);
                } else {
                    count = Integer.valueOf(count.intValue() + 1);
                }
                distinctValues.put(integer, count);
            }
        }
        /**
    ...
    private boolean checkDistinctValuesAndEncodeWithPopulationCodec(String name, int[] band, BHSDCodec defaultCodec, BandData bandData, BandAnalysisResults results) throws Pack200Exception {
        ...
        if (effort > 3 && !name.equals("POPULATION") &&
                (numDistinctValues < 100 || distinctValuesAsProportion < 0.02 ||
                        (effort > 6 && distinctValuesAsProportion < 0.04))) {
            encodeWithPopulationCodec(name, band, defaultCodec, bandData, results);
            return timeToStop(results);
        }
    ...
    private BandAnalysisResults analyseBand(final String name, final int[] band, final BHSDCodec defaultCodec)
        ...
        checkDistinctValuesAndEncodeWithPopulationCodec(name, band, defaultCodec, bandData, results);
