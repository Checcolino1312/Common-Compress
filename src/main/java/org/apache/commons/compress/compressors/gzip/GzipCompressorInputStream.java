...
public class GzipCompressorInputStream extends CompressorInputStream
    ...
    private void validateSkipAmount(int skipAmount) throws IOException {
        if (IOUtils.skip(in, skipAmount) != skipAmount) {
            throw new IOException();
        }
    }

    private void validateCRC32(long crcStored) throws IOException {
        if (crcStored != crc.getValue()) {
            throw new IOException("Gzip-compressed data is corrupt (CRC32 error)");
        }
    }

    private void handleInputIfNeeded() throws IOException {
        if (inf.needsInput()) {
            handleInput();
        }
    }
    ...
    private void validateUncompressedSize(long isize) throws IOException {
