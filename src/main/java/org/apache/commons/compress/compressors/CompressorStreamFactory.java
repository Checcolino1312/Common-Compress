...
public class CompressorStreamFactory implements CompressorStreamProvider {
    ...
    public CompressorInputStream createCompressorInputStream(final String name, final InputStream in,
        ...
        try {
            CompressorInputStream compressorInputStream = createCompressorByType(name, in, actualDecompressConcatenated);
    ...
    private CompressorInputStream createCompressorByType(final String name, final InputStream in,
                                                         final boolean actualDecompressConcatenated) throws CompressorException, IOException {
        ...
        if (BROTLI.equalsIgnoreCase(name)) {
            checkBrotliCompressionAvailability();
            return new BrotliCompressorInputStream(in);
        }

        if (XZ.equalsIgnoreCase(name)) {
            checkXZCompressionAvailability();
            return new XZCompressorInputStream(in, actualDecompressConcatenated, memoryLimitInKb);
        }

        if (ZSTANDARD.equalsIgnoreCase(name)) {
            checkZstdCompressionAvailability();
            return new ZstdCompressorInputStream(in);
        }

        if (LZMA.equalsIgnoreCase(name)) {
            checkLZMACompressionAvailability();
            return new LZMACompressorInputStream(in, memoryLimitInKb);
        }

        if (PACK200.equalsIgnoreCase(name)) return new Pack200CompressorInputStream(in);
        if (SNAPPY_RAW.equalsIgnoreCase(name)) return new SnappyCompressorInputStream(in);
        if (SNAPPY_FRAMED.equalsIgnoreCase(name)) return new FramedSnappyCompressorInputStream(in);
        if (Z.equalsIgnoreCase(name)) return new ZCompressorInputStream(in, memoryLimitInKb);
        if (DEFLATE.equalsIgnoreCase(name)) return new DeflateCompressorInputStream(in);
        if (DEFLATE64.equalsIgnoreCase(name)) return new Deflate64CompressorInputStream(in);
        if (LZ4_BLOCK.equalsIgnoreCase(name)) return new BlockLZ4CompressorInputStream(in);
        if (LZ4_FRAMED.equalsIgnoreCase(name)) return new FramedLZ4CompressorInputStream(in, actualDecompressConcatenated);

        return null;
    }

    private void checkBrotliCompressionAvailability() throws CompressorException {
        if (!BrotliUtils.isBrotliCompressionAvailable()) {
            throw new CompressorException("Brotli compression is not available." + YOU_NEED_BROTLI_DEC);
        }
    }

    private void checkXZCompressionAvailability() throws CompressorException {
        if (!XZUtils.isXZCompressionAvailable()) {
            throw new CompressorException("XZ compression is not available." + YOU_NEED_XZ_JAVA);
        }
    }

    private void checkZstdCompressionAvailability() throws CompressorException {
        if (!ZstdUtils.isZstdCompressionAvailable()) {
            throw new CompressorException("Zstandard compression is not available." + YOU_NEED_ZSTD_JNI);
        }
    }

    private void checkLZMACompressionAvailability() throws CompressorException {
