...
public class BZip2CompressorOutputStream extends CompressorOutputStream
    ...
    private static void hbMakeCodeLengths(final byte[] len, final int[] freq,
        ...
        while (tooLong) {
            ...
            while (nHeap > 1) {
                ...
                heap[1] = heap[nHeap];
                nHeap--;
                heapifyDown(heap, weight, nHeap, 1);
            ...
            processAlphaSize(parent, len, alphaSize, maxLen, weight);


        }
    }

    private static void heapifyUp(int[] heap, int[] weight, int i) {
        int tmp = heap[i];
        int tmpWeight = weight[tmp];
        while (tmpWeight < weight[heap[i >> 1]]) {
            heap[i] = heap[i >> 1];
            i >>= 1;
        }
        heap[i] = tmp;
    ...
    private static void heapifyDown(int[] heap, int[] weight, int nHeap, int i) {
        int tmp = heap[i];
        int tmpWeight = weight[tmp];
        while (true) {
            int yy = i << 1;
    ...
    private static void processAlphaSize(int[] parent, byte[] len, int alphaSize, int maxLen, int[] weight) {
