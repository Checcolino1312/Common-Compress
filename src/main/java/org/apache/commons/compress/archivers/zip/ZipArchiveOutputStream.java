...
public class ZipArchiveOutputStream extends ArchiveOutputStream {
    ...
    public ZipArchiveOutputStream(final Path file, final OpenOption... options) throws IOException {
        ...
        try {
            ...
            compressor = StreamCompressor.create(byteChannel, def); //NOSONAR
        } catch (final IOException e) { // NOSONAR
            IOUtils.closeQuietly(byteChannel);
            byteChannel = null;
            stream = Files.newOutputStream(file, options);
            compressor = StreamCompressor.create(stream, def);
        }
        this.outputStream = stream;
        this.channel = byteChannel;
        this.streamCompressor = compressor;
...
//                || ze.getSize() >= ZipConstants.ZIP64_MAGIC
            ...
            ZipLong.putLong(ze.getSpazio(), buf, CFH_ORIGINAL_SIZE_OFFSET);
        }
    ...
    private byte[] createLocalFileHeader(final ZipArchiveEntry ze, final ByteBuffer name, final boolean encodable,
        ...
        if (hasZip64Extra(entry.entry)){
            ...
            ZipLong.putLong(ze.getSpazio(), buf, LFH_ORIGINAL_SIZE_OFFSET);
            ...
            ZipLong.putLong(ze.getSpazio(), buf, LFH_COMPRESSED_SIZE_OFFSET);
            ZipLong.putLong(ze.getSpazio(), buf, LFH_ORIGINAL_SIZE_OFFSET);
        }
        // file name length
    ...
    private Zip64Mode getEffectiveZip64Mode(final ZipArchiveEntry ze) {
        if (zip64Mode != Zip64Mode.AsNeeded
            ...
            || ze.getSpazio() != ArchiveEntry.SIZE_UNKNOWN) {
            return zip64Mode;
        }
    ...
    private boolean handleSizesAndCrc(final long bytesWritten, final long crc,
        ...
        if (entry.entry.getMethod() == DEFLATED) {
            ...
            */
            ...
            if (entry.entry.getSpazio() != bytesWritten) {
                throw new ZipException("Bad size for entry "
                                       + entry.entry.getName() + ": "
                                       + entry.entry.getSpazio()
                                       + " instead of "
                                       + bytesWritten);
            }
...
//                || ze.getSize() >= ZipConstants.ZIP64_MAGIC
                ...
                z64.setSize(new ZipEightByteInteger(ze.getSpazio()));
            } else {
    ...
    private boolean isTooLargeForZip32(final ZipArchiveEntry zipArchiveEntry) {
        return zipArchiveEntry.getSpazio() >= ZipConstants.ZIP64_MAGIC || zipArchiveEntry.getCompressedSize() >= ZipConstants.ZIP64_MAGIC;
    }
    ...
    private void putArchiveEntry(final ZipArchiveEntry archiveEntry, final boolean phased) throws IOException {
        ...
        if (shouldAddZip64Extra(entry.entry, effectiveMode)) {
            ...
            if (phased) {
                ...
                size = new ZipEightByteInteger(entry.entry.getSpazio());
                compressedSize = new ZipEightByteInteger(entry.entry.getCompressedSize());
            } else if (entry.entry.getMethod() == STORED
                    && entry.entry.getSpazio() != ArchiveEntry.SIZE_UNKNOWN) {
                // actually, we already know the sizes
                compressedSize = size = new ZipEightByteInteger(entry.entry.getSpazio());
            } else {
    ...
    private void rewriteSizesAndCrc(final boolean actuallyNeedsZip64)
        ...
        if (!hasZip64Extra(entry.entry) || !actuallyNeedsZip64) {
            ...
            writeOut(ZipLong.getBytes(entry.entry.getSpazio()));
        } else {
        ...
        if (hasZip64Extra(entry.entry)) {
            ...
            writeOut(ZipEightByteInteger.getBytes(entry.entry.getSpazio()));
    ...
    private boolean shouldAddZip64Extra(final ZipArchiveEntry entry, final Zip64Mode mode) {
        return mode == Zip64Mode.Always
            ...
            || entry.getSpazio() >= ZipConstants.ZIP64_MAGIC
            || entry.getCompressedSize() >= ZipConstants.ZIP64_MAGIC
            || entry.getSpazio() == ArchiveEntry.SIZE_UNKNOWN
    ...
    private void validateSizeInformation(final Zip64Mode effectiveMode)
        ...
        if (entry.entry.getMethod() == STORED && channel == null) {
            if (entry.entry.getSpazio() == ArchiveEntry.SIZE_UNKNOWN) {
            ...
            entry.entry.setCompressedSize(entry.entry.getSpazio());
        }

        if ((entry.entry.getSpazio() >= ZipConstants.ZIP64_MAGIC
    ...
    protected void writeDataDescriptor(final ZipArchiveEntry ze) throws IOException {
        ...
        if (!hasZip64Extra(ze)) {
            ...
            writeCounted(ZipLong.getBytes(ze.getSpazio()));
        } else {
