...
public class DumpArchiveInputStream extends ArchiveInputStream {
    ...
    private DumpArchiveEntry readNextEntry() throws IOException {
        DumpArchiveEntry entry = null;
        byte[] headerBytes;

        while (entry == null) {
            readIdx = processPreviousSegment(readIdx);
            ...
            if (active.getHeaderType() == DumpArchiveConstants.SEGMENT_TYPE.END) {
                hasHitEOF = true;
                return null;
            }
    ...
    private int processPreviousSegment(int index) throws IOException {
        while (index < active.getHeaderCount()) {
            if (!active.isSparseRecord(index) && raw.skip(DumpArchiveConstants.TP_SIZE) == -1) {
    ...
    private void readDirectoryEntry(DumpArchiveEntry entry) throws IOException {
        ...
        while (shouldContinueReadingDirectoryEntry(entry, first)) {
            readHeader(first);
            processEntry(entry);

            int datalen = DumpArchiveConstants.TP_SIZE * entry.getHeaderCount();
            readBlockBuffer(datalen);
            ...
            for (int i = 0; i < datalen - 8 && i < size - 8; i += reclen) {
                ...
                processDirent(ino, entry, type, name);
            ...
            peekBytes = raw.peek();
            verifyPeekBytes(peekBytes);

            entry = getNextEntry(peekBytes);
            first = false;
            size -= DumpArchiveConstants.TP_SIZE;
        }
    }

    private boolean shouldContinueReadingDirectoryEntry(DumpArchiveEntry entry, boolean first) {
        return first || DumpArchiveConstants.SEGMENT_TYPE.ADDR == entry.getHeaderType();
    }

    private void readHeader(boolean first) throws IOException {
        if (!first) {
            raw.readRecord();
        }
    }

    private void processEntry(DumpArchiveEntry entry) {
        if (!names.containsKey(entry.getIno()) && DumpArchiveConstants.SEGMENT_TYPE.INODE == entry.getHeaderType()) {
            pending.put(entry.getIno(), entry);
        }
    }

    private void readBlockBuffer(int datalen) throws IOException {
    ...
    private void processDirent(int ino, DumpArchiveEntry entry, byte type, String name) {
    ...
    private void verifyPeekBytes(byte[] peekBytes) throws InvalidFormatException {
        if (!DumpArchiveUtil.verify(peekBytes)) {
            throw new InvalidFormatException();
        }
    }

    private DumpArchiveEntry getNextEntry(byte[] peekBytes) {
        return DumpArchiveEntry.parse(peekBytes);
    }
