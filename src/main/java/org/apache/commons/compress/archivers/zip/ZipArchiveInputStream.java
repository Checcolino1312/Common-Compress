...
public class ZipArchiveInputStream extends ArchiveInputStream implements InputStreamStatistics {
    ...
    */
    ...
    private boolean closeAndCheckEntry(boolean firstEntry) throws IOException {
        if (current != null) {
            closeEntry();
            return false;
        }
    ...
    private void processUTF8Flag(boolean hasUTF8Flag, ZipArchiveEntry entry) {
        if (hasUTF8Flag) {
    ...
    public ZipArchiveEntry getNextZipEntry() throws IOException {
        ...
        firstEntry = closeAndCheckEntry(firstEntry);
        ...
        processUTF8Flag(hasUTF8Flag, current.entry);
        ...
        processZipMethod(m, inputStream, current.entry);

        entriesRead++;
        return current.entry;
    }
    private void processZipMethod(ZipMethod m, InputStream inputStream, ArchiveEntry entry) throws IOException {
        if (entry.getSize() != ArchiveEntry.SIZE_UNKNOWN && entry instanceof ZipArchiveEntry) {
            ZipArchiveEntry zipEntry = (ZipArchiveEntry) entry;
            if (ZipUtil.canHandleEntryData(zipEntry) && m != ZipMethod.STORED && m != ZipMethod.DEFLATED) {
                final InputStream bis = new BoundedInputStream(inputStream, zipEntry.getCompressedSize());
                switch (m) {
                    case UNSHRINKING:
        ...
        } else if (m == ZipMethod.ENHANCED_DEFLATED || entry.getSize() == ArchiveEntry.SIZE_UNKNOWN) {
