...
public class SevenZFile implements Closeable {
    ...
    private static int assertFitsIntoNonNegativeInt(final String what, final long value) throws IOException {
        if (value > Integer.MAX_VALUE || value < 0) {
            throw new IOException(String.format("Cannot handle %s,d",what));
        }
        return (int) value;
    }
            ...
            switch (propertyType) {
                ...
                case NID.kEmptyFile: {
                    if (isEmptyStream != null) {
                        ...
                        isEmptyFile = new BitSet();
                    }
                    break;
                }
                case NID.kAnti: {
                    if (isEmptyStream != null) { // Verifica se isEmptyStream è nullo
                        isAnti = readBits(header, isEmptyStream.cardinality());
                    } else {
                        throw new IOException("isEmptyStream is null");
                    }
                    break;
                }
                case NID.kName: {
    ...
    private void sanityCheckFilesInfo(final ByteBuffer header, final ArchiveStatistics stats) throws IOException {
        ...
        while (true) {
            ...
            switch (propertyType) {
                case NID.kEmptyStream:
                    emptyStreams = readEmptyStream(header, stats.numberOfEntries);
                    break;
                case NID.kEmptyFile:
                case NID.kAnti:
                    checkEmptyStreamsAvailability(emptyStreams, "kEmptyStream must appear before " + getPropertyTypeName(propertyType));
                    readBits(header, emptyStreams);
                    break;
                case NID.kName:
                    checkNameProperty(header, size, stats.numberOfEntries);
                    break;
                case NID.kCTime:
                case NID.kATime:
                case NID.kMTime:
                    checkTimeProperty(header, stats.numberOfEntries, getPropertyTypeName(propertyType));
                    break;
                case NID.kWinAttributes:
                    checkWinAttributesProperty(header, stats.numberOfEntries);
                    break;
                case NID.kStartPos:
                    throw new IOException("kStartPos is unsupported, please report");
                case NID.kDummy:
                    skipDummyProperty(header, size);
                    break;
                default:
                    skipProperty(header, size, propertyType);
                    break;
            }
        }
    ...
    private int readEmptyStream(final ByteBuffer header, final int numberOfEntries) throws IOException {
        try {
            return readBits(header, numberOfEntries).cardinality();
        } catch (IOException e) {
            // Handle or rethrow the exception as per your requirement
            throw new IOException("Error reading empty stream", e);
        }
    }


    private void checkEmptyStreamsAvailability(final int emptyStreams, final String errorMessage) throws IOException {
        if (emptyStreams == -1) {
            throw new IOException("Header format error: " + errorMessage);
        }
    }

    private void checkNameProperty(final ByteBuffer header, final long size, final int numberOfEntries) throws IOException {
        final int external = getUnsignedByte(header);
        if (external != 0) {
        ...
        if (filesSeen != numberOfEntries) {
            throw new IOException("Invalid number of file names (" + filesSeen + " instead of " + numberOfEntries + ")");
        }
    }

    private void checkTimeProperty(final ByteBuffer header, final int numberOfEntries, final String propertyName) throws IOException {
        final int timesDefined = readAllOrBits(header, numberOfEntries).cardinality();
        final int external = getUnsignedByte(header);
        if (external != 0) {
            throw new IOException("Not implemented");
        }
        if (skipBytesFully(header, (long) 8 * timesDefined) < 8 * timesDefined) {
            throw new IOException("invalid " + propertyName + " dates size");
        }
    }

    private void checkWinAttributesProperty(final ByteBuffer header, final int numberOfEntries) throws IOException {
        final int attributesDefined = readAllOrBits(header, numberOfEntries).cardinality();
        final int external = getUnsignedByte(header);
        if (external != 0) {
            throw new IOException("Not implemented");
        }
        if (skipBytesFully(header, (long) 4 * attributesDefined) < 4 * attributesDefined) {
            throw new IOException("invalid windows attributes size");
        }
    }

    private void skipDummyProperty(final ByteBuffer header, final long size) throws IOException {
        if (skipBytesFully(header, size) < size) {
            throw new IOException("Incomplete kDummy property");
        }
    }

    private void skipProperty(final ByteBuffer header, final long size, final int propertyType) throws IOException {
        if (skipBytesFully(header, size) < size) {
            throw new IOException("Incomplete property of type " + propertyType);
        }
    }

    private String getPropertyTypeName(final int propertyType) {
        switch (propertyType) {
            case NID.kEmptyStream:
                return "kEmptyStream";
            case NID.kEmptyFile:
    ...
    private void checkNumCoders(int numCoders) throws IOException {
        if (numCoders == 0) {
            throw new IOException("Folder without coders");
        }
    }
    private void checkForAlternativeMethods(boolean moreAlternativeMethods) throws IOException {
        if (moreAlternativeMethods) {
    ...
    private void processAttributes(boolean hasAttributes, ByteBuffer header) throws IOException {
        if (hasAttributes) {
    ...
    private void checkOutIndex(int outIndex, long totalOutStreams) throws IOException {
    ...
    private int sanityCheckFolder(final ByteBuffer header, final ArchiveStatistics stats)
        ...
        for (int i = 0; i < numCoders; i++) {
            ...
            final boolean hasAttributes = (bits & 0x20) != 0;
            final boolean moreAlternativeMethods = (bits & 0x80) != 0;
            checkForAlternativeMethods(moreAlternativeMethods);
            ...
            processAttributes(hasAttributes, header);
        }
    ...
    private void checkSinglePackedStream(int numPackedStreams, BitSet inStreamsBound) throws IOException {
        if (numPackedStreams == 1 && (inStreamsBound.nextClearBit(0) == -1)) {
    ...
    private void sanityCheckPackInfo(final ByteBuffer header, final ArchiveStatistics stats) throws IOException {
        ...
        if (nid == NID.kCRC) {
            long crcsDefined = (long) readAllOrBits(header, stats.numberOfPackedStreams)
                .cardinality();
    ...
    private void sanityCheckSubStreamsInfo(final ByteBuffer header, final ArchiveStatistics stats) throws IOException {
        ...
        if (nid == NID.kNumUnpackStream) {
            numUnpackSubStreamsPerFolder.addAll(readNumUnpackSubStreamsPerFolder(header, stats));
            stats.numberOfUnpackSubStreams = calculateNumberOfUnpackSubStreams(numUnpackSubStreamsPerFolder);
            nid = getUnsignedByte(header);
        } else {
        ...
        if (nid == NID.kSize) {
            processSizes(header, numUnpackSubStreamsPerFolder);
            nid = getUnsignedByte(header);
        }

        final int numDigests = calculateNumDigests(stats, numUnpackSubStreamsPerFolder);

        if (nid == NID.kCRC) {
            assertFitsIntoNonNegativeInt("numDigests", numDigests);
            validateMissingCrcs(header, numDigests);
            nid = getUnsignedByte(header);
        }
    ...
    private List<Integer> readNumUnpackSubStreamsPerFolder(final ByteBuffer header, final ArchiveStatistics stats) throws IOException {
        final List<Integer> numUnpackSubStreamsPerFolder = new ArrayList<>();
        ...
        return numUnpackSubStreamsPerFolder;
    }

    private long calculateNumberOfUnpackSubStreams(final List<Integer> numUnpackSubStreamsPerFolder) {
        return numUnpackSubStreamsPerFolder.stream().mapToLong(Integer::longValue).sum();
    }

    private void processSizes(final ByteBuffer header, final List<Integer> numUnpackSubStreamsPerFolder) throws IOException {
    ...
    private int calculateNumDigests(final ArchiveStatistics stats, final List<Integer> numUnpackSubStreamsPerFolder) {
        int numDigests = 0;
    ...
    private void validateMissingCrcs(final ByteBuffer header, final int numDigests) throws IOException {
        assertFitsIntoNonNegativeInt("numDigests", numDigests);
        long missingCrcs = (long) readAllOrBits(header, numDigests).cardinality();
    ...
    private void sanityCheckUnpackInfo(final ByteBuffer header, final ArchiveStatistics stats)
        ...
        if (nid == NID.kCRC) {
            ...
            long crcsDefined = (long) stats.folderHasCrc.cardinality();
            if (skipBytesFully(header, 4 * crcsDefined) < 4 * crcsDefined) {
