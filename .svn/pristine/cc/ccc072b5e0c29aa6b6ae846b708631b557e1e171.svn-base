...
public class ClassBands extends BandSet {
	...
	public void addAnnotation(final int context, final String desc, final boolean visible, final List<String> nameRU,
		...
		switch (context) {
			case MetadataBandGroup.CONTEXT_CLASS:
				handleClassAnnotation(visible, desc, nameRU, tags, values, caseArrayN, nestTypeRS, nestNameRU, nestPairN);
				break;
			case MetadataBandGroup.CONTEXT_FIELD:
				handleFieldAnnotation(visible, desc, nameRU, tags, values, caseArrayN, nestTypeRS, nestNameRU, nestPairN);
				break;
			case MetadataBandGroup.CONTEXT_METHOD:
				handleMethodAnnotation(visible, desc, nameRU, tags, values, caseArrayN, nestTypeRS, nestNameRU, nestPairN);
				break;
		}
	}

	private void handleClassAnnotation(final boolean visible, final String desc, final List<String> nameRU,
									   final List<String> tags, final List<Object> values, final List<Integer> caseArrayN,
									   final List<String> nestTypeRS, final List<String> nestNameRU, final List<Integer> nestPairN) {
		if (visible) {
	...
	private void handleFieldAnnotation(final boolean visible, final String desc, final List<String> nameRU,
									   final List<String> tags, final List<Object> values, final List<Integer> caseArrayN,
									   final List<String> nestTypeRS, final List<String> nestNameRU, final List<Integer> nestPairN) {
		if (visible) {
	...
	private void handleMethodAnnotation(final boolean visible, final String desc, final List<String> nameRU,
										final List<String> tags, final List<Object> values, final List<Integer> caseArrayN,
										final List<String> nestTypeRS, final List<String> nestNameRU, final List<Integer> nestPairN) {
		if (visible) {
...
//		}
	...
	private static final String PREFIXX =  "Wrote ";
	...
	public void pack(final OutputStream out) throws IOException, Pack200Exception {
		...
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from class_this[" + class_this.length + "]");

		encodedBand = encodeBandInt("class_super", getInts(class_super), Codec.DELTA5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from class_super[" + class_super.length + "]");

		encodedBand = encodeBandInt("class_interface_count", class_interface_count, Codec.DELTA5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from class_interface_count["
		...
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from class_interface[" + classInterface.length + "]");

		encodedBand = encodeBandInt("class_field_count", class_field_count, Codec.DELTA5);
		out.write(encodedBand);
		PackingUtils
				.log( PREFIXX + encodedBand.length + " bytes from class_field_count[" + class_field_count.length + "]");

		encodedBand = encodeBandInt("class_method_count", class_method_count, Codec.DELTA5);
		out.write(encodedBand);
		PackingUtils.log(
				PREFIXX + encodedBand.length + " bytes from class_method_count[" + class_method_count.length + "]");
		...
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from field_descr[" + fieldDescr.length + "]");
		...
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from method_descr[" + methodDescr.length + "]");
	...
	public void removeCurrentClass() {
		removeClassSourceFile();
		removeClassEnclosingMethod();
		removeClassSignature();
		removeClassRVAAndRIABands();
		removeFieldFlags();
		removeMethodFlags();

		resetClassProperties();
		clearTemporaryFieldsAndMethods();
		decrementIndexIfGreaterThanZero();
	}

	private void removeClassSourceFile() {
		if ((class_flags[index] & (1 << 17)) != 0) {
			classSourceFile.remove(classSourceFile.size() - 1);
		}
	}

	private void removeClassEnclosingMethod() {
	...
	private void removeClassSignature() {
		if ((class_flags[index] & (1 << 19)) != 0) {
			classSignature.remove(classSignature.size() - 1);
		}
	}

	private void removeClassRVAAndRIABands() {
		if ((class_flags[index] & (1 << 21)) != 0) {
	...
	private void removeMethodFlags() {
		Map<Long, Runnable> flagActions = new HashMap<>();

		flagActions.put(1L << 19, () -> methodSignature.remove(methodSignature.size() - 1));
		flagActions.put(1L << 18, this::removeMethodExceptions);
		flagActions.put(1L << 17, this::removeCodeAttributes);
		flagActions.put(1L << 21, method_RVA_bands::removeLatest);
		flagActions.put(1L << 22, method_RIA_bands::removeLatest);
		flagActions.put(1L << 23, method_RVPA_bands::removeLatest);
		flagActions.put(1L << 24, method_RIPA_bands::removeLatest);
		flagActions.put(1L << 25, method_AD_bands::removeLatest);

		for (final Long flagsL : tempMethodFlags) {
			final long flags = flagsL.longValue();
			for (Map.Entry<Long, Runnable> entry : flagActions.entrySet()) {
	...
	private void removeMethodExceptions() {
	...
	private void removeCodeAttributes() {
	...
	private void removeCodeDebugAttributes() {
		...
		if ((cdeFlags & (1 << 3)) != 0) {
			removeCodeLocalVariablesInTypeTable();
		}
		if ((cdeFlags & (1 << 1)) != 0) {
	...
	private void removeCodeLocalVariablesInTypeTable() {
	...
	private void removeCodeLineNumbers() {
	...
	private void resetClassProperties() {
	...
	private void clearTemporaryFieldsAndMethods() {
		tempFieldDesc.clear();
		tempFieldFlags.clear();
		tempMethodDesc.clear();
		tempMethodFlags.clear();
	...
	private void decrementIndexIfGreaterThanZero() {
		if (index > 0) {
			index--;
		}
	}
	...
	private void writeClassAttributeBands(final OutputStream out) throws IOException, Pack200Exception {
		...
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from class_flags[" + class_flags.length + "]");
		...
		out.write(encodedBand);
		PackingUtils
				.log( PREFIXX + encodedBand.length + " bytes from class_attr_calls[" + class_attr_calls.length + "]");

		encodedBand = encodeBandInt("classSourceFile", cpEntryOrNullListToArray(classSourceFile), Codec.UNSIGNED5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from classSourceFile[" + classSourceFile.size() + "]");

		encodedBand = encodeBandInt("class_enclosing_method_RC", cpEntryListToArray(classEnclosingMethodClass),
				Codec.UNSIGNED5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from class_enclosing_method_RC["
		...
		encodedBand = encodeBandInt("class_EnclosingMethod_RDN", cpEntryOrNullListToArray(classEnclosingMethodDesc),
				Codec.UNSIGNED5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from class_EnclosingMethod_RDN["
		...
		out.write(encodedBand);
		PackingUtils
				.log( PREFIXX + encodedBand.length + " bytes from class_Signature_RS[" + classSignature.size() + "]");
		...
		encodedBand = encodeBandInt("class_InnerClasses_outer_RCN", cpEntryOrNullListToArray(classInnerClassesOuterRCN),
				Codec.UNSIGNED5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from class_InnerClasses_outer_RCN["
		...
		encodedBand = encodeBandInt("class_InnerClasses_name_RUN", cpEntryOrNullListToArray(classInnerClassesNameRUN),
				Codec.UNSIGNED5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from class_InnerClasses_name_RUN["
		...
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from classFileVersionMinor["
				+ classFileVersionMinor.size() + "]");

		encodedBand = encodeBandInt("classFileVersionMajor", classFileVersionMajor.toArray(), Codec.UNSIGNED5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from classFileVersionMajor["
	...
	private void writeCodeAttributeBands(final OutputStream out) throws IOException, Pack200Exception {
		...
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from codeFlags[" + codeFlags.size() + "]");
		...
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from code_attr_calls[" + code_attr_calls.length + "]");

		encodedBand = encodeBandInt("code_LineNumberTable_N", codeLineNumberTableN.toArray(), Codec.UNSIGNED5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from code_LineNumberTable_N["
				+ codeLineNumberTableN.size() + "]");

		encodedBand = encodeBandInt("code_LineNumberTable_bci_P", integerListToArray(codeLineNumberTableBciP),
				Codec.BCI5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from code_LineNumberTable_bci_P["
		...
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from code_LineNumberTable_line["
		...
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from code_LocalVariableTable_N["
		...
		encodedBand = encodeBandInt("code_LocalVariableTable_bci_P", integerListToArray(codeLocalVariableTableBciP),
				Codec.BCI5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from code_LocalVariableTable_bci_P["
		...
		encodedBand = encodeBandInt("code_LocalVariableTable_span_O", integerListToArray(codeLocalVariableTableSpanO),
				Codec.BRANCH5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from code_LocalVariableTable_span_O["
		...
		encodedBand = encodeBandInt("code_LocalVariableTable_name_RU", cpEntryListToArray(codeLocalVariableTableNameRU),
				Codec.UNSIGNED5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from code_LocalVariableTable_name_RU["
		...
		encodedBand = encodeBandInt("code_LocalVariableTable_type_RS", cpEntryListToArray(codeLocalVariableTableTypeRS),
				Codec.UNSIGNED5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from code_LocalVariableTable_type_RS["
		...
		encodedBand = encodeBandInt("code_LocalVariableTable_slot", codeLocalVariableTableSlot.toArray(),
				Codec.UNSIGNED5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from code_LocalVariableTable_slot["
		...
		encodedBand = encodeBandInt("code_LocalVariableTypeTable_N", codeLocalVariableTypeTableN.toArray(),
				Codec.UNSIGNED5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from code_LocalVariableTypeTable_N["
		...
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from code_LocalVariableTypeTable_bci_P["
		...
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from code_LocalVariableTypeTable_span_O["
		...
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from code_LocalVariableTypeTable_name_RU["
		...
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from code_LocalVariableTypeTable_type_RS["
		...
		encodedBand = encodeBandInt("code_LocalVariableTypeTable_slot", codeLocalVariableTypeTableSlot.toArray(),
				Codec.UNSIGNED5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from code_LocalVariableTypeTable_slot["
	...
	private void writeCodeBands(final OutputStream out) throws IOException, Pack200Exception {
		...
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from codeHeaders[" + codeHeaders.length + "]");

		encodedBand = encodeBandInt("codeMaxStack", codeMaxStack.toArray(), Codec.UNSIGNED5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from codeMaxStack[" + codeMaxStack.size() + "]");

		encodedBand = encodeBandInt("codeMaxLocals", codeMaxLocals.toArray(), Codec.UNSIGNED5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from codeMaxLocals[" + codeMaxLocals.size() + "]");

		encodedBand = encodeBandInt("codeHandlerCount", codeHandlerCount.toArray(), Codec.UNSIGNED5);
		out.write(encodedBand);
		PackingUtils
				.log( PREFIXX + encodedBand.length + " bytes from codeHandlerCount[" + codeHandlerCount.size() + "]");

		encodedBand = encodeBandInt("codeHandlerStartP", integerListToArray(codeHandlerStartP), Codec.BCI5);
		out.write(encodedBand);
		PackingUtils
				.log( PREFIXX + encodedBand.length + " bytes from codeHandlerStartP[" + codeHandlerStartP.size() + "]");

		encodedBand = encodeBandInt("codeHandlerEndPO", integerListToArray(codeHandlerEndPO), Codec.BRANCH5);
		out.write(encodedBand);
		PackingUtils
				.log( PREFIXX + encodedBand.length + " bytes from codeHandlerEndPO[" + codeHandlerEndPO.size() + "]");

		encodedBand = encodeBandInt("codeHandlerCatchPO", integerListToArray(codeHandlerCatchPO), Codec.BRANCH5);
		out.write(encodedBand);
		PackingUtils.log(
				PREFIXX + encodedBand.length + " bytes from codeHandlerCatchPO[" + codeHandlerCatchPO.size() + "]");

		encodedBand = encodeBandInt("codeHandlerClass", cpEntryOrNullListToArray(codeHandlerClass), Codec.UNSIGNED5);
		out.write(encodedBand);
		PackingUtils
				.log( PREFIXX + encodedBand.length + " bytes from codeHandlerClass[" + codeHandlerClass.size() + "]");
	...
	private void writeFieldAttributeBands(final OutputStream out) throws IOException, Pack200Exception {
		...
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from field_flags[" + field_flags.length + "]");
		...
		out.write(encodedBand);
		PackingUtils
				.log( PREFIXX + encodedBand.length + " bytes from field_attr_calls[" + field_attr_calls.length + "]");

		encodedBand = encodeBandInt("fieldConstantValueKQ", cpEntryListToArray(fieldConstantValueKQ), Codec.UNSIGNED5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from fieldConstantValueKQ["
				+ fieldConstantValueKQ.size() + "]");

		encodedBand = encodeBandInt("fieldSignature", cpEntryListToArray(fieldSignature), Codec.UNSIGNED5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from fieldSignature[" + fieldSignature.size() + "]");
	...
	private void writeMethodAttributeBands(final OutputStream out) throws IOException, Pack200Exception {
		...
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from method_flags[" + method_flags.length + "]");
		...
		out.write(encodedBand);
		PackingUtils
				.log( PREFIXX + encodedBand.length + " bytes from method_attr_calls[" + method_attr_calls.length + "]");

		encodedBand = encodeBandInt("methodExceptionNumber", methodExceptionNumber.toArray(), Codec.UNSIGNED5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from methodExceptionNumber["
				+ methodExceptionNumber.size() + "]");

		encodedBand = encodeBandInt("methodExceptionClasses", cpEntryListToArray(methodExceptionClasses),
				Codec.UNSIGNED5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from methodExceptionClasses["
				+ methodExceptionClasses.size() + "]");

		encodedBand = encodeBandInt("methodSignature", cpEntryListToArray(methodSignature), Codec.UNSIGNED5);
		out.write(encodedBand);
		PackingUtils.log( PREFIXX + encodedBand.length + " bytes from methodSignature[" + methodSignature.size() + "]");
