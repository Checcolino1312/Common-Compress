...
public class PopulationCodec extends Codec {
    ...
    private boolean checkBreakCondition(int k, int value, int smallest, int last) {
        return k > -1 && (value == smallest || value == last);
    }

    private void checkTokenCodec(Codec tokenCodec, Object k, Object l) throws Pack200Exception {
        if (tokenCodec == null) {
            throw new Pack200Exception("Cannot calculate token codec from " + k + " and " + l);
        }
    }

    private void checkTokenCodec() {
        int k = -1;
        if (tokenCodec == null) {
            if (k >= 256) {
                findBHSDCodec();
    ...
    public int[] decodeInts(final int n, final InputStream in) throws IOException, Pack200Exception {
        ...
        while (true) {
            ...
            if (checkBreakCondition(k, value, smallest, last)) {
                break;
            }
            favoured[++k] = value;
        ...
        checkTokenCodec(tokenCodec, k, l);
        // read favorites
        lastBandLength += n;
