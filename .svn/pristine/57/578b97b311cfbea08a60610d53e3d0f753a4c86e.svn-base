...
public final class FramedLZ4CompressorInputStreamTest extends AbstractTestCase {
    ...
    private void expectIOException(final String fileName) {
        try (InputStream is = Files.newInputStream(getFile(fileName).toPath())) {
            ...
            assertThrows(IOException.class, () -> IOUtils.toByteArray(in), "Expected IOException");
        } catch (IOException e) {
    ...
    public void rejectsBlocksWithoutChecksum() {
        ...
        assertNotNull(ex);
        assertEquals("Premature end of stream while reading block checksum", ex.getMessage());
    }
    ...
    public void rejectsNonLZ4Stream() {
        assertThrows(IOException.class, this::createFramedLZ4CompressorInputStream);
    }

    private void createFramedLZ4CompressorInputStream() throws IOException {
        try (InputStream inputStream = new FileInputStream("bla.tar")) {
    ...
    public void rejectsSkippableFrameFollowedByJunk() {
        ...
        assertNotNull(ex);
        assertEquals("LZ4 frame stream followed by garbage", ex.getMessage());
    }
    ...
    public void rejectsSkippableFrameWithPrematureEnd() {
        ...
        assertNotNull(ex);
        assertEquals("Premature end of stream while skipping frame", ex.getMessage());
    }
    ...
    public void rejectsSkippableFrameWithPrematureEndInLengthBytes() {
        ...
        assertNotNull(ex);
        assertEquals("Premature end of data", ex.getMessage());
    }
    ...
    public void rejectsStreamsWithBadContentChecksum() {
        ...
        assertNotNull(ex);
        assertEquals("content checksum mismatch.", ex.getMessage());
    }
    ...
    public void rejectsStreamsWithoutContentChecksum() {
        ...
        assertNotNull(ex);
        assertEquals("Premature end of stream while reading content checksum", ex.getMessage());
    }
