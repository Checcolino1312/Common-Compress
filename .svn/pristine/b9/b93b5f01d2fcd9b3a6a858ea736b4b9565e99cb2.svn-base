...
public class ChangeSetPerformer {
    ...
    private ChangeSetResults perform(final ArchiveEntryIterator entryIterator, final ArchiveOutputStream out) throws IOException {
        ...
        doNothingMethod(results, workingSet, out);
        ...
        addRemainingFiles(workingSet, out, results);

        out.finish();
        return results;
    }

    private void doNothingMethod(ChangeSetResults results, Set<Change> workingSet, ArchiveOutputStream out) {
        for (final Iterator<Change> it = workingSet.iterator(); it.hasNext();) {
            final Change change = it.next();
            ...
            if (change.type() == Change.TYPE_ADD && change.isReplaceMode()) {
                try {
                    ...
                    // Gestisci l'eccezione o propagala a livelli superiori
                    // Inserisci il codice appropriato qui...
                }
            }
        }
    }

    private void addRemainingFiles(Set<Change> workingSet, ArchiveOutputStream out, ChangeSetResults results) throws IOException {
        for (final Iterator<Change> it = workingSet.iterator(); it.hasNext();) {
            final Change change = it.next();
