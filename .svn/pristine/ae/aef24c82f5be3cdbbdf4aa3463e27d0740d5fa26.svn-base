...
public class NewAttributeBands extends BandSet {
    ...
    public class Union extends LayoutElement {
        ...
        public void addToAttribute(final int n, final NewAttribute attribute) {
            ...
            for (final UnionCase unionCase : unionCases) {
                if (unionCase.hasTag(tag)) {
                    defaultCase = false;
                    offset = calculateOffset(n, tagBand, unionCase);
                    unionCase.addToAttribute(offset, attribute);
                }
            }

            if (defaultCase) {
                handleDefaultCase(n, tagBand, attribute);
            }
        }

        private int calculateOffset(int n, int[] tagBand, UnionCase unionCase) {
            int offset = 0;
            for (int j = 0; j < n; j++) {
        ...
        private void handleDefaultCase(int n, int[] tagBand, NewAttribute attribute) {
            int defaultOffset = calculateDefaultOffset(n, tagBand);
            if (defaultCaseBody != null) {
                applyAttributesToDefaultCase(defaultOffset, attribute);
            }
        }

        private int calculateDefaultOffset(int n, int[] tagBand) {
            int defaultOffset = 0;
            for (int j = 0; j < n; j++) {
                boolean found = false;
                for (final UnionCase unionCase : unionCases) {
        ...
        private void applyAttributesToDefaultCase(int defaultOffset, NewAttribute attribute) {
    ...
    private LayoutElement readNextLayoutElement(final StringReader stream) throws IOException {
        ...
        case 'T':
            ...
            int openingParentheses = stream.read(); // '('
            int closingParentheses = stream.read(); // ')'
            int openingBracket = stream.read(); // '['
    ...
    private UnionCase readNextUnionCase(final StringReader stream) throws IOException {
        stream.mark(2);
        int parentheses = stream.read(); // '('
        if (parentheses == -1) {
            // Gestisci il caso in cui la lettura restituisca -1
            // ...
        }
    ...
    private int resolveCallsForElement(final int i, final Callable currentCallable, final LayoutElement layoutElement) {
        ...
        if (layoutElement instanceof Call) {
            ...
            if (index == 0) { // Calls the parent callable
                ...
                int foundIndex = findCallableIndex(i + 1, attributeLayoutElements, index);
                if (foundIndex != -1) {
                    call.setCallable((Callable) attributeLayoutElements.get(foundIndex));
                }
            } else { // Backwards call
                int foundIndex = findCallableIndexReverse(i - 1, attributeLayoutElements, index);
                if (foundIndex != -1) {
                    call.setCallable((Callable) attributeLayoutElements.get(foundIndex));
                }
            }
    ...
    private int findCallableIndex(int startIndex, List<AttributeLayoutElement> elements, int index) {
        int count = 0;
        for (int k = startIndex; k < elements.size(); k++) {
            final AttributeLayoutElement el = elements.get(k);
    ...
    private int findCallableIndexReverse(int startIndex, List<AttributeLayoutElement> elements, int index) {
        int count = 0;
