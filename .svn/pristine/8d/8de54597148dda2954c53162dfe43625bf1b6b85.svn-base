...
public class ZipArchiveEntry extends java.util.zip.ZipEntry implements ArchiveEntry, EntryStreamOffsets {
     ...
     *        >COMPRESS-93</a>
     */
    private int compressionMethod = ZipMethod.UNKNOWN_CODE;
    /**
     ...
     */
    private long sizer = SIZE_UNKNOWN;
    ...
    public String getName() {
        return nameR == null ? super.getName() : nameR;
    }
    ...
    private ZipExtraField getExistingExtraField(final ZipExtraField element) {
        if (element instanceof UnparseableExtraFieldData) {
            return unparseableExtra;
        } else {
            return getExtraField(element.getHeaderId());
        }
    }

    private void handleExistingExtraField(final ZipExtraField existing, final byte[] b, final boolean local) {
        try {
            if (local) {
            ...
            handleZipException(existing, b, local);
        }
    }

    private void handleZipException(final ZipExtraField existing, final byte[] b, final boolean local) {
        final UnrecognizedExtraField u = createUnrecognizedExtraField(existing, b, local);
        internalRemoveExtraField(existing.getHeaderId());
        internalAddExtraField(u);
    }

    private UnrecognizedExtraField createUnrecognizedExtraField(final ZipExtraField existing, final byte[] b, final boolean local) {
        final UnrecognizedExtraField u = new UnrecognizedExtraField();
     ...
     * @param nameR the name to use
     */
    protected void setName(String nameR) {
        if (nameR != null && getPlatform() == PLATFORM_FAT
                && !nameR.contains("/")) {
            nameR = nameR.replace('\\', '/');
        }
        this.nameR = nameR;
